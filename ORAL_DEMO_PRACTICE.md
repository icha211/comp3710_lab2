...
CNN test metrics (loss, accuracy): [0.6402, 0.7671]
```

Saved figure: [lab2/Figure_lfw.png](lab2/Figure_lfw.png)

Say:

"The classical PCA plus Random Forest baseline reached 55.3% accuracy on the held-out test set. Looking at the classification report, it collapsed onto the majority class: George W. Bush has 133 of 322 test images, and the model predicts him with perfect recall but the minority identities like Ariel Sharon and Hugo Chavez get zero recall. This is a class imbalance problem, not just a weak model. After 10 epochs, the CNN reached 76.7% test accuracy, a clear improvement over the PCA baseline, because it learns spatial filters directly from the 2D image rather than relying on unsupervised PCA directions, which are not guaranteed to separate identities. The training curves in Figure_lfw.png show accuracy rising and loss falling smoothly across both training and validation, with validation accuracy tracking training accuracy closely, so there's no strong sign of overfitting yet at 10 epochs."
...
### Part 1, DFT (1 Mark) — Now implemented, not yet run
...
- TF tensor-based `naive_dft_tf` (DFT-matrix matmul formulation, since looping in Python over GPU tensors is not how you vectorise this) plus a CPU-vs-GPU-vs-`tf.signal.fft` timing sweep across sizes 256/512/1024/2048: **implemented** in `run_dft_tensor`, callable via `python lab2_solution.py --part dft-tensor`. **Not yet run on this machine or on Rangpur** — no GPU here, so GPU timings will show as unavailable locally; must be executed on Rangpur for the real comparison.

### Part 2, Eigenfaces (1 Mark) — Now implemented, not yet run

- The original `run_lfw` still uses `sklearn.PCA`, kept because it has verified real results (55.3%/76.7%).
- Added `run_eigenfaces_manual`, matching the task sheet exactly: manual `np.linalg.svd` PCA, eigenface gallery plot, compactness (cumulative explained variance) plot, and `RandomForestClassifier(max_features=150)`. Run via `python lab2_solution.py --part eigenfaces`. **Not yet executed** — no real accuracy number for this exact version yet.

### Part 3.1, CNN Classifier (1 Mark) — Done and verified

Unchanged, matches spec, real run output already captured (76.7% test accuracy at 10 epochs).
...
- Code functions and completes tasks, hosted on GitHub, results explained: DFT/LFW/CNN/binary-U-Net are done and verified; the TF-tensor DFT, manual eigenfaces, DAWNBench, VAE, OASIS U-Net, and GAN paths are now implemented but still need an actual run (locally where possible, and on Rangpur for anything OASIS/CIFAR/GPU-timing related) before you can claim their results.
...
"DFT, PCA/Random-Forest versus CNN on LFW, and the binary U-Net warm-up are fully run and I have real numbers for them. I've now also implemented the TF-tensor GPU DFT comparison, the exact-spec manual-SVD eigenfaces pipeline, the DAWNBench ResNet-18 CIFAR-10 trainer, and all three Part 4 tasks — VAE, multi-class OASIS U-Net with per-label DSC, and a GAN — but none of those six have been executed yet, and the OASIS-specific ones still need to be pointed at the real dataset on Rangpur before I can report genuine results."
