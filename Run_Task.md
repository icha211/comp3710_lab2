...
python lab2_solution.py --part oasis-unet \
  --data-root /home/groups/comp3710/OASIS \
  --epochs 10 --num-classes 4 --output-dir demo_outputs
```
...
The final test output must show mean DSC above `0.9` for labels 1, 2, and 3. Explain that the network has four softmax channels, the masks are one-hot encoded, categorical cross-entropy is used, and predictions are converted back to labels with `argmax` for discrete DSC.
